...
本包使用官方 `realsense2_camera_node` 采集，并支持每个机器人配置任意数量的 RealSense。D405 使用共享 `depth_module`，D435 使用独立的 `depth_module` 与 `rgb_camera`。没有开流后切换、参数回读或逐帧曝光检查。
...
`multi_camera.launch.py` 接受管理器部署在机器人组合目录中的 `cameras.yaml`。同时启用多台 RealSense 时每台必须填写唯一序列号。D435 彩色流默认使用 4500 μs 手动曝光；若在网页中启用彩色自动曝光，当前配置无法提供 5 ms 上限承诺。
