...
独立的 `timestamp_adapter.py` 把官方 metadata 报告的曝光中点转换到 ROS 时间，并为点云保留来源深度/RGB 关联。设备时间字段的物理准确性仍需独立验证。它不调用相机 SDK，也不修改官方驱动。`humanoid_manager` 只保存机器人相机清单并生成 launch 输入，相机采集仍在独立节点中执行。

适配器、验收器及管理器的图像录制接收端优先请求 `RELIABLE`，发现发布端仅提供 `BEST_EFFORT` 时兼容回退；适配器的标准化图像、元数据及点云输出使用 `RELIABLE`。使用 `VOLATILE` 与有界队列，避免启动时接收历史图像。诊断和验收报告记录实际接收策略。此策略支持重传，但不能保证负载过高时零丢帧；需以真机连续采集验收为准。

相机图像和元数据发布默认显式使用官方 `DEFAULT` 预设（RELIABLE、KEEP_LAST 10、VOLATILE）。官方图像默认的 `SYSTEM_DEFAULT` 在 Humble/Fast DDS 默认环境下实际可只保留 1 帧，丢失分片后发送端可能已覆盖待重传的图像。对应参数为根级 `depth_qos`、`color_qos`、`depth_info_qos`、`color_info_qos`，不带 `depth_module` / `rgb_camera` 前缀。显式高级参数选择予以保留；自定义单相机 YAML 也可覆盖预设。验收会回读这四项，旧进程未重启时会提示与新配置不符。更改发布策略后必须正常重启相机，单独执行 `ros2 param set` 不会立即重建发布端。此修复不改变采集分辨率、帧率、曝光、滤波或时间戳公式。

标准化点云或点云元数据有下游订阅者时，适配器才订阅原始点云，发现周期约 1 秒；停止订阅后释放点云缓存。点云等待配对使用独立缓存（最多 4 帧及 16 MiB，且不超过配置内存上限），避免淘汰 RGBD 等待组。不改变相机的点云开关、分辨率、帧率、曝光或滤波配置。更新后需同时重启相机适配进程及管理器录制进程。
...
需要照片证据时加 `--save-evidence`，默认每台相机每隔 10 秒抽样、最多保存 6 对 RGB/深度照片，并生成包含照片的 HTML 报告及 ZIP。RGB 保存为无损 PNG，深度 PNG 保留收到的 16 位像素数值，另附显示预览。每对照片附带逐帧来源元数据、曝光、增益、时间戳和 SHA-256；元数据未匹配时明确标记未知。失败或采样中按 Ctrl+C 也保留已收到的照片和部分结果。此选项同时启用图像验证，增加 RGBD 接收及后台写盘开销，不修改相机参数、发布 QoS 或发布内容。
