...
- `frame_events.csv`：每台相机的原始 RGB/深度元数据、标准化元数据、RGBD 图像接收记录；同样保存在 `report.json` 的 `frame_trace` 中。

逐帧接收记录用于区分“检查器没收到某帧元数据”和“没有收到对应图像”。它用标准化元数据中的
来源帧号关联 RGBD 图像的外层及两路内层 Header，只核对原始元数据首尾帧号之间的缺口。
报告单独列出其中仍有对应图像的帧号，不修改已有 FAIL 判定；其余缺口表示尚未定位，不能直接认定相机漏拍。
每台最多保留最近 20000 条小型记录，不额外订阅图像、不复制图像像素；超限或时钟 epoch 改变时停止缺口归类并明确标注。
`receive_ns` 是检查器记录的 ROS 接收时刻，`adapter_group_ns` 是适配节点四份图像/元数据凑齐后的时刻，
SDK 到达字段保留原单位 ms。这些阶段时刻不等于物理曝光时刻。旧报告无法补回未记录的逐帧信息。
...
适配节点的输入接收速率低，说明问题发生在它收到输入之前（含驱动发布、DDS 传输及回调调度）；
输入接收足够但成组发布低时，检查不完整组淘汰、Header 配对及无效元数据；
节点成组发布足够但检查器接收低时，再检查输出传输及检查器调度。
这些计数不能单独区分 USB 丢帧和 DDS 丢包，也不能当作硬件帧率。

更新相机包后，正在运行的旧适配进程不会自动加载新代码，需在下次正常重启相机进程后采样。
旧版节点没有诊断话题时，验收仍继续，报告注明缺少诊断窗口。
诊断本身只增加计数和每秒一条小消息。当前版本还记录适配器输入 QoS、标准化输出 QoS、RMW 实现及点云转发状态。
